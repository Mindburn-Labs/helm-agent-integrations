...
class TestParseArgv(unittest.TestCase):
    ...
    def test_read_verbs(self):
        ...
        self.assertClass(["get", "pods", "-n", "mindburn-qa"], "read_only", namespace="mindburn-qa")
    ...
    def test_global_flags_before_verb(self):
        intent = self.assertClass(
            ["--namespace", "prod", "get", "pods"], "read_only", verb="get", namespace="prod"
        )
    ...
    def test_dry_run_downgrades_to_read_only(self):
        intent = self.assertClass(["apply", "-f", "x.yaml", "--dry-run=client"], "read_only")
        self.assertTrue(intent.dry_run)
        intent = self.assertClass(["delete", "pod", "x", "--dry-run=server"], "read_only")
        self.assertTrue(intent.dry_run)
    ...
    def test_summary(self):
        ...
        self.assertEqual(intent.summary, "delete pod nginx -n prod")
...
def make_config(tmp: Path, mode: str = "enforce", real: str = "/usr/local/bin/kubectl") -> guard.GuardConfig:
...
class TestEvaluate(unittest.TestCase):
    ...
    def test_allow_verdict_parsed(self):
        body = {"decision": {"verdict": "ALLOW", "reason_code": "KUBECTL_READ_ONLY_ALLOW", "receipt_id": "r-1", "decision_id": "d-1"}}
        with mock.patch.object(guard.urllib.request, "urlopen", return_value=FakeResponse(body)):
    ...
    def test_header_refs_preferred(self):
        body = {"verdict": "DENY"}
        headers = {"X-Helm-Receipt-Id": "r-h", "X-Helm-Reason-Code": "KUBECTL_DESTRUCTIVE_DENY"}
        with mock.patch.object(guard.urllib.request, "urlopen", return_value=FakeResponse(body, headers)):
    ...
    def test_missing_verdict_defaults_deny(self):
        with mock.patch.object(guard.urllib.request, "urlopen", return_value=FakeResponse({})):
            verdict = guard.evaluate(self.config, intent_for(["get", "pods"]))
        self.assertEqual(verdict.verdict, "DENY")

    def test_unexpected_verdict_raises(self):
        with mock.patch.object(guard.urllib.request, "urlopen", return_value=FakeResponse({"verdict": "MAYBE"})):
            with self.assertRaises(guard.GuardError):
                guard.evaluate(self.config, intent_for(["get", "pods"]))

    def test_http_error_raises(self):
        err = guard.urllib.error.HTTPError("u", 401, "unauthorized", {}, io.BytesIO(b"{}"))
    ...
    def test_approval_ref_forwarded(self):
        ...
        def spy(request, timeout=0):
            ...
            return FakeResponse({"decision": {"verdict": "ALLOW"}})
...
class TestMainFlow(unittest.TestCase):
    def setUp(self):
        ...
        self.env = {
            ...
            "HELM_KUBECTL_REAL": "/usr/local/bin/kubectl",
    ...
    def run_main(self, argv, body, env_extra=None):
        ...
        with mock.patch.object(guard.urllib.request, "urlopen", return_value=FakeResponse(body)):
            with mock.patch.object(guard.os, "execv") as exec_mock:
                code = guard.main(argv, env)
        return code, exec_mock

    def test_allow_dispatches(self):
        code, exec_mock = self.run_main(["get", "pods"], {"decision": {"verdict": "ALLOW", "receipt_id": "r"}})
        self.assertEqual(code, 127)  # main returns dispatch()'s fallback
        exec_mock.assert_called_once()
        args = exec_mock.call_args[0]
        self.assertEqual(args[0], "/usr/local/bin/kubectl")
        self.assertEqual(args[1], ["/usr/local/bin/kubectl", "get", "pods"])
    ...
    def test_deny_blocks(self):
        code, exec_mock = self.run_main(
            ...
            {"decision": {"verdict": "DENY", "reason_code": "KUBECTL_DESTRUCTIVE_DENY"}},
        )
    ...
    def test_escalate_blocks_with_exit_2(self):
        code, exec_mock = self.run_main(
            ["apply", "-f", "deploy.yaml"],
            {"decision": {"verdict": "ESCALATE", "reason_code": "KUBECTL_MUTATION_APPROVAL_REQUIRED", "decision_id": "d-7"}},
        )
...
class TestRealKubectlResolution(unittest.TestCase):
    def test_skips_shim_directory(self):
        with tempfile.TemporaryDirectory() as shim_dir, tempfile.TemporaryDirectory() as real_dir:
            ...
            self.assertEqual(guard.resolve_real_kubectl(env, shim_dir), str(real_path))

    def test_override_wins(self):
        env = {"HELM_KUBECTL_REAL": "/opt/real/kubectl", "PATH": ""}
        self.assertEqual(guard.resolve_real_kubectl(env, "/x"), "/opt/real/kubectl")
